...
const collaborator = "agenthackathon";
const repositories = [
...
for (const repository of repositories) {
  ...
  if (![201, 204].includes(collaboratorResponse.status)) throw new Error(`Admin collaborator grant failed for ${repository.name} (HTTP ${collaboratorResponse.status}).`);
